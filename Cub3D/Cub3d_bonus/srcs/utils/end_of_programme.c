...
/*   Updated: 2025/11/12 15:57:15 by ebenoist         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	free_end_of_programme3(t_game *game)
{
	...
	while (i < 4)
	{
		if (game->texture.img_alt[i])
		{
			mlx_destroy_image(game->mlx.mlx_ptr, game->texture.img_alt[i]);
			game->texture.img_alt[i] = NULL;
			game->texture.alt_tex_ptr[i].data = NULL;
		}
		i++;
	}
	if (game->texture.no2)
	{
		mlx_destroy_image(game->mlx.mlx_ptr, game->texture.no2);
		game->texture.no2 = NULL;
	}
	if (game->mlx.w_ptr)
